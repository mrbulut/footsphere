...
class PostRatings{
  ...
  const
    ...
    PROJECT_URI  = 'https://digitalnature.eu/forum/plugins/post-ratings/',   // url to support forums
  ...
  */
  ...
  final public function loadTemplate($_name, $_vars = array()) {
    ...
    if(!$_located)
      $_located = SHOPME_INC_PLUGINS_PATH .'post-ratings/templates/' . $_name . '.php';
  ...
  */
  public function CreateMenu(){
    add_theme_page(esc_html__('Post Ratings', 'shopme'), esc_html__('Post Ratings', 'shopme'), 'manage_options', self::ID, array($this, 'SettingsPage'), 62);
  }
  ...
  */
  public function PluginSettingsLink($links, $file){
    ...
    if (SHOPME_INC_PLUGINS_URI . 'post-ratings/' === $file) {
      $settings_link = '<a href="'. esc_url(add_query_arg(array('page' => self::ID), admin_url('options-general.php'))) .'">'.esc_html__('Settings', 'shopme').'</a>';
  ...
  */
  public function SettingsPage(){
    ...
    $generic_pages = array(
      'home'     => esc_html__('Home', 'shopme'),
      'archive'  => esc_html__('Archives', 'shopme'),
      'singular' => esc_html__('Single pages', 'shopme'),
      'search'   => esc_html__('Search results', 'shopme'),
    );

    ?>
    <div class="wrap metabox-holder">
      <h2><?php esc_html_e('Post Ratings', 'shopme'); ?></h2>

      <form method="post" action="options.php" style="position:relative;">

        <div class="postbox" style="position:absolute;right:0;top:0;">
          <h3><span><?php esc_html_e('Global stats', 'shopme'); ?></span></h3>
          <div class="inside">
            <p><?php printf(wp_kses(__('%1$s votes (on %2$s posts)', 'shopme'), 'default'), sprintf('<strong>%d</strong>', $num_votes), sprintf('<strong>%d</strong>', $num_rated_posts)); ?></p>
            <p><?php printf(wp_kses(__('Average vote count per post: %s', 'shopme'), 'default'), sprintf('<strong>%.2F</strong>', @($options['num_votes'] / $options['num_rated_posts']))); ?></p>
            <p><?php printf(wp_kses(__('Average rating per post: %s', 'shopme'), 'default'), sprintf('<strong>%.2F</strong>', $avg_rating)); ?></p>
          </div>
        </div>
        ...
        <table class="form-table">
          ...
          <tr valign="top">
            <th scope="row"><?php esc_html_e('Access level', 'shopme'); ?></th>
            <td>
              <label for="anonymous_vote">
                ...
                <?php esc_html_e('Allow unregistered users to vote', 'shopme'); ?>
             </label>
            </td>
          </tr>

          <tr valign="top">
            <th scope="row"><?php esc_html_e('Maximum rating', 'shopme'); ?></th>
            <td>
              <input type="text" size="3" name="<?php echo self::ID; ?>[max_rating]" value="<?php echo $max_rating; ?>" />
              <p><span class="description"><?php esc_html_e('Changing this option will reset existing post rating records', 'shopme'); ?></span></p>
            </td>
          </tr>

          <tr valign="top">
            <th scope="row">
              <?php esc_html_e('Bayesian rating (score) formula', 'shopme'); ?>
            </th>
            <td>
              ...
              <fieldset>
               ...
               <label for="bayesian_formula_1">
                 <input id="bayesian_formula_1" name="<?php echo self::ID; ?>[bayesian_formula]" type="radio" value="<?php echo self::BR1; ?>" <?php checked($bayesian_formula, self::BR1); ?> />
                  ...
                  <code style="font-size: 14px;">(<em>v</em> / (<em>v</em> + <strong>MV</strong>)) * <em>r</em> + (<strong>MV</strong> / (<em>v</em> + <strong>MV</strong>)) * <strong>R</strong></code> (<?php printf(wp_kses(__('from %s', 'shopme'), array('a' => array('href' => array(), 'target' => array()))), '<a href="https://en.wikipedia.org/wiki/Internet_Movie_Database#User_ratings_of_films" target="_blank">IMDB</a>'); ?>)
               </label>
               </p>
               ...
               <label for="bayesian_formula_2">
                 ...
                 <code style="font-size: 14px;">((<strong>AV</strong> * <strong>R</strong>) + (<em>v</em> * <em>r</em>)) / (<strong>AV</strong> + <em>v</em>)</code> (<?php printf(wp_kses(__('from %s', 'shopme'), array('a' => array('href' => array(), 'target' => array()))), '<a href="https://gist.github.com/44522/" target="_blank">thebroth</a>'); ?>)
               </label>
               </p>
               ...
               <label for="user_formula">
                 ...
                 <?php esc_html_e('I have my own formula:', 'shopme'); ?>
               ...
               </label> <a href="#" onclick="jQuery('#legend').toggle();">(<?php esc_html_e('Legend', 'shopme'); ?>)</a>
               </p>

               <div id="legend" style="display:none;">
                <p>
                 <code style="font-size: 14px;"><strong>AV</strong></code> = <?php esc_html_e('Global average number of votes per post', 'shopme'); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>&nbsp;V</strong></code> = <?php esc_html_e('Global number of votes (from all posts)', 'shopme'); ?>
                 <br />
                 <code style="font-size: 14px;"><em>&nbsp;v</em></code> = <?php esc_html_e('Number of votes from the current post', 'shopme'); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>&nbsp;R</strong></code> = <?php printf(esc_html__('Global average rating per post (from 1 to %d)', 'shopme'), $max_rating); ?>
                 <br />
                 <code style="font-size: 14px;"><em>&nbsp;r</em></code> = <?php printf(esc_html__('Average rating of the current post (from 1 to %d)', 'shopme'), $max_rating); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>MV</strong></code> = <?php printf(esc_html__('Minimum vote count per post to consider (%d by default)', 'shopme'), self::MIN_VOTES); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>MR</strong></code> = <?php printf(esc_html__('Maximum rating, see option above (currently %d)', 'shopme'), $max_rating); ?>
               </p>
              </div>
          ...
          <tr valign="top">
            <th scope="row"><?php esc_html_e('Allow ratings on', 'shopme'); ?></th>
            <td>
              <fieldset>
          ...
          <tr valign="top">
            <th scope="row"><?php esc_html_e('Locations of the rate links', 'shopme'); ?></th>
            <td>
              ...
              <fieldset>
               ...
               <label for="custom_filter">
                  <?php _e('I have my own action hook:', 'shopme'); ?>
                  <input id="custom_filter" class="code" type="text" value="<?php echo $custom_filter; ?>" name="<?php echo self::ID; ?>[custom_filter]" size="40" />
               </label>
              </fieldset>

              <p>
                <span class="description"><?php printf(esc_html__('You can also add it manually anywhere by using the %s shortcode', 'shopme'), '<code>[rate]</code>'); ?></span>
              </p>
          ...
          <tr><td colspan="2">&nbsp;</td></tr>
            ...
            <th scope="row" colspan="2">
              ...
              <input type="submit" class="button-primary" value="<?php esc_html_e('Save Changes', 'shopme'); ?>" />
              &nbsp;
              <label for="remove_all">
                <input id="remove_all" type="checkbox" value="1" name="<?php echo self::ID; ?>[remove_ratings]" />
                <?php esc_html_e('Delete rating records from all posts', 'shopme'); ?>
              </label>
      ...
      <div style="background:#eee;padding: 5px 10px;margin: 10px 5px;">
        <?php printf(esc_html__('Found a bug, having a feature request or just looking for help on using this plugin? Then head on to the %s.', 'shopme'), '<a href="'. esc_url(self::PROJECT_URI) .'">'.__('Post Ratings support forums', 'shopme').'</a>'); ?>
      </div>
    </div>
    <?php
  }
  ...
  */
  public function assets(){
    ...
    // js
    wp_enqueue_script( SHOPME_PREFIX . self::ID, SHOPME_INC_PLUGINS_URI . 'post-ratings/post-ratings.js', array('jquery'), self::VERSION, true );
    ...
    if(!class_exists('Atom') || (class_exists('Atom') && (!defined('Atom::VERSION'))))
      wp_localize_script( SHOPME_PREFIX . self::ID, 'post_ratings', array('blog_url' => home_url('/')) );

    // allow themes to override css
    $style = SHOPME_INC_PLUGINS_URI . 'post-ratings/post-ratings.css';

    wp_enqueue_style( SHOPME_PREFIX . self::ID, $style );
  }
...
//      }
    ...
    // this is our $.ajax request
      ...
      if(!$post){
        $error = esc_html__("Invalid vote! Cheatin' uh?", 'shopme');
        ...
        if($this->currentUserCanRate($post_id)){
          ...
          $error = esc_html__('You cannot rate this post!', 'shopme');
        }
  ...
  */
  public function ControlBlockHook($content = ''){
    ...
    if($control){
      ...
      if(empty($post->ID)){
        printf(esc_html__("Your '%s' action is must run in a post's context!", 'shopme'), $custom_filter);
        return $content;
      }
