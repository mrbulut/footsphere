...
<head>
	...
	<link rel="stylesheet" href="<?php echo esc_url(SHOPME_BASE_URI . 'css/fontello.css') ?>" />
	<link rel="stylesheet" href="<?php echo esc_url(SHOPME_COMPARE_URI . '/colorbox.css') ?>"/>
	<link rel="stylesheet" href="<?php echo esc_url(SHOPME_COMPARE_URI . '/jquery.dataTables.css') ?>"/>
	...
	if ( get_option('exists_stylesheet'. $prefix_name ) == true ) {
		...
		?>
		<link rel="stylesheet" href="<?php echo $upload_dir['baseurl'] . '/dynamic_shopme_dir/' . $prefix_name . '.css' ?>">
	<?php } ?>

	<?php if (is_rtl()): ?>
		<link rel="stylesheet" href="<?php echo esc_url(SHOPME_COMPARE_URI . '/rtl.css') ?>" type="text/css" />
	<?php endif; ?>
	...
	<style type="text/css">
		body.loading {
			background: url("<?php echo SHOPME_BASE_URI ?>images/ajax-loader@2x.gif") no-repeat scroll center center transparent;
		}
	</style>
</head>
<!-- END HEAD -->
...
<h2>
	...
	<?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'shopme' ) ?></a><?php endif; ?>
</h2>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
	...
	<?php if ( empty( $products ) ) : ?>
		...
		<tr class="no-products">
			<td><?php esc_html_e( 'No products added in the compare table.', 'shopme' ) ?></td>
		</tr>
						...
						switch( $field ) {
							...
							case 'add-to-cart':
								...
								echo '<div class="remove">';
									echo '<a href="' . esc_url(add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) )) . '" data-product_id="' . $product_id.'"></a>';
								echo '</div>';
								break;
							default:
