<?php
	...
	if ( is_singular('post') || is_category() || is_tag() || is_archive() ) {
		...
		$post_id = shopme_custom_get_option('blog_get_widgets_page_id');

		if ( !$post_id ) {
			$posts_page = get_page_by_title( 'Home 1' );
	...
	if ( shopme_is_product() || shopme_is_product_category() || shopme_is_product_tag() || is_tax( get_object_taxonomies( 'product' ) ) || is_post_type_archive('product') ) {
		$shop_post_id = shopme_custom_get_option('shop_get_widgets_page_id');
		if ( $shop_post_id ) {
			list($footer_row_top_show, $footer_row_middle_show, $footer_row_bottom_show, $get_sidebars_top_widgets, $get_sidebars_middle_widgets, $get_sidebars_bottom_widgets, $footer_row_top_columns_variations, $footer_row_middle_columns_variations, $footer_row_bottom_columns_variations) = SHOPME_WIDGETS_META_BOX::get_settings_from_post_meta($shop_post_id);
		}
	}

	if ( is_404() ) {
		$error_post_id = shopme_custom_get_option('error_get_widgets_page_id');
		if ( $error_post_id ) {
			list($footer_row_top_show, $footer_row_middle_show, $footer_row_bottom_show, $get_sidebars_top_widgets, $get_sidebars_middle_widgets, $get_sidebars_bottom_widgets, $footer_row_top_columns_variations, $footer_row_middle_columns_variations, $footer_row_bottom_columns_variations) = SHOPME_WIDGETS_META_BOX::get_settings_from_post_meta($error_post_id);
		}
	}

	if ( is_search() ) {
		$search_post_id = shopme_custom_get_option('search_get_widgets_page_id');
		if ( $search_post_id ) {
			list($footer_row_top_show, $footer_row_middle_show, $footer_row_bottom_show, $get_sidebars_top_widgets, $get_sidebars_middle_widgets, $get_sidebars_bottom_widgets, $footer_row_top_columns_variations, $footer_row_middle_columns_variations, $footer_row_bottom_columns_variations) = SHOPME_WIDGETS_META_BOX::get_settings_from_post_meta($search_post_id);
		}
	}
